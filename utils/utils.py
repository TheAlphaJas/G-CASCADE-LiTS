...
def calculate_hd95_percase(pred, gt):
    pred[pred > 0] = 1
    gt[gt > 0] = 1
    if pred.sum() > 0 and gt.sum()>0:
        dice = metric.binary.hd95(pred, gt)
        return dice
    elif pred.sum() > 0 and gt.sum()==0:
        return 0
    else:
...
def calculate_jaccard_percase(pred, gt):
    pred[pred > 0] = 1
    gt[gt > 0] = 1
    if pred.sum() > 0 and gt.sum()>0:
        dice = metric.binary.jc(pred, gt)
        return dice
    elif pred.sum() > 0 and gt.sum()==0:
        return 1
    else:
...
def test_lits_single(image, label, net, classes, patch_size=[256, 256], test_save_path=None, case=None, z_spacing=1):
    image, label = image.squeeze(0).cpu().detach().numpy(), label.squeeze(0).cpu().detach().numpy()
    ...
    metric_list = []
    for i in range(1, classes):
      z1 = calculate_dice_percase(prediction == i, label == i)
      z2 = calculate_hd95_percase(prediction == i, label == i)
      z3 = calculate_jaccard_percase(prediction == i, label == i)
      metric_list.append([z1,z2,z3])
    return metric_list
